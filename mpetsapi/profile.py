...
async def profile(pet_id, cookies, timeout, connector):
    try:
        ...
        last_login = "online"
        async with ClientSession(cookies=cookies, timeout=ClientTimeout(
                total=timeout), connector=connector) as session:
...
async def view_profile(pet_id, cookies, connector):
    try:
        ...
        async with ClientSession(cookies=cookies,
            ...
            resp = await session.get("http://mpets.mobi/view_profile",
                                     params=params)
