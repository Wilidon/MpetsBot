...
@simple_bot_message_handler(holidays_router,
                            PayloadFilter({"command": "0308"}))
async def holiday_handler(event: SimpleBotEvent):
    holiday = False
    text = ""
    current_user = event["current_user"]
    today = int(datetime.today().strftime("%m%d"))
    if holiday_308[0] <= today <= holiday_308[1]:
        text = f"🌹 Международный женский день\n\n"
        holiday = True
    current_user_tasks = crud.get_user_tasks(user_id=current_user.user_id, today=holiday_308[2])
    if not current_user_tasks and holiday is True:
        await functions.creation_womenDay_tasks(user=current_user, date=holiday_308)
    current_user_tasks = crud.get_user_tasks(user_id=current_user.user_id, today=holiday_308[2])
    counter = 1
    for task in current_user_tasks:
        task_name, progress, end = task.task_name, task.progress, task.end
        ...
        if progress >= end:
            task_name = task_name.split("_", maxsplit=1)[0]
            text += f"{counter}. " + holiday_0308_completed[
                task_name].format(*args) + "Выполнено ✅\n\n "
            counter += 1
        else:
            if "avatar" in task_name or "anketa" in task_name:
            ...
            args = [progress, end]
            task_name = task_name.split("_", maxsplit=1)[0]
            text += f"{counter}. " + holiday_0308[task_name].format(*args) + "\n"
            counter += 1
    # время до конца мероприятия
    today = str(holiday_308[1] + 1) + datetime.today().strftime("%Y")
    left_time = time.mktime(datetime.strptime(today, "%m%d%Y").timetuple())
    left_time = int(left_time) - int(time.time())
    if left_time > 0:
        text += f"\n До окончания заданий {await timer(left_time)}"
...
@simple_bot_message_handler(holidays_router,
                            PayloadFilter({"command": "0401"}))
async def holiday_handler(event: SimpleBotEvent):
    holiday = False
    text = ""
    current_user = event["current_user"]
    today = int(datetime.today().strftime("%m%d"))
    if holiday_401[0] <= today <= holiday_401[1]:
        text = f"❗ Процесс удаления игры\n\n"
        holiday = True
    current_user_tasks = crud.get_user_tasks(user_id=current_user.user_id, today=holiday_401[2])
    if not current_user_tasks and holiday is True:
        await functions.creation_firstApril_tasks(user=current_user, date=holiday_401)
    current_user_tasks = crud.get_user_tasks(user_id=current_user.user_id, today=holiday_401[2])
    counter = 1
    for task in current_user_tasks:
        task_name, progress, end = task.task_name, task.progress, task.end
        ...
        if progress >= end:
            task_name = task_name.split("_", maxsplit=1)[0]
            text += f"{counter}. " + holiday_0401_completed[
                task_name].format(*args) + "Выполнено ✅\n\n "
            counter += 1
        else:
            if "avatar" in task_name or "anketa" in task_name:
            ...
            args = [progress, end]
            task_name = task_name.split("_", maxsplit=1)[0]
            text += f"{counter}. " + holiday_0401[task_name].format(*args) + "\n"
            counter += 1
    # время до конца мероприятия
    today = str(holiday_401[1] + 1) + datetime.today().strftime("%Y")
    left_time = time.mktime(datetime.strptime(today, "%m%d%Y").timetuple())
    left_time = int(left_time) - int(time.time())
    if left_time > 0:
        text += f"\n До окончания заданий {await timer(left_time)}"
